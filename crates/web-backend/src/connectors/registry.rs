...
pub fn builtin_registry() -> ConnectorTypeRegistry {
    ...
    types.insert(
        ...
        ConnectorTypeRegistration {
            ...
            connection_test_factory: Some(|config| {
                Box::pin(async move {
                    GoveeConnector::from_config_value_for_connection_test(config)
                        .map(|connector| Box::new(connector) as Box<dyn Connector>)
                })
            }),
    ...
    types.insert(
        ...
        ConnectorTypeRegistration {
            ...
            connection_test_factory: Some(|config| {
                Box::pin(async move {
                    TasmotaConnector::from_config_value_for_connection_test(config)
                        .map(|connector| Box::new(connector) as Box<dyn Connector>)
                })
            }),
            schema: loom_connector_tasmota::config_schema(),
            setup_guide: Some(loom_connector_tasmota::setup_guide()),
            discoverable_type: None,
            discovery_target_field: None,
        },
    );
...
mod tests {
    ...
    fn the_tasmota_type_is_registered_with_an_optional_encrypted_password() {
        ...
        let guide = registration.setup_guide.as_ref().expect("setup guide");
        assert_eq!(guide.variants.len(), 1);
        assert_eq!(guide.variants[0].id, "device-address");
        assert!(registration.connection_test_factory.is_some());
        assert!(registration.discoverable_type.is_none());
    }
