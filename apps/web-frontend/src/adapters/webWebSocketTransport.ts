...
class BrowserTransportSocket implements TransportSocket {
...
/** Web keeps using the browser WebSocket API; only its ownership moved. */
export const webWebSocketTransport: WebSocketTransport = {
  async connect(url) {
    return new BrowserTransportSocket(new WebSocket(url));
  },
};
