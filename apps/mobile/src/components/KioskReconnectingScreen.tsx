 ...
 */
export function KioskReconnectingScreen({
  serverBaseUrl,
  verifying = false,
}: {
  serverBaseUrl: string;
  /** The session check is still in flight rather than being retried. */
  verifying?: boolean;
}) {
  return (
    <main
    ...
    >
      ...
      <h1 className="text-lg font-semibold">{verifying ? "Starting…" : "Reconnecting…"}</h1>
      <p className="text-sm text-muted-foreground">
        {verifying
          ? "This display is checking its saved session."
