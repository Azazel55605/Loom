...
| connector-govee | `pnpm build:connector-govee` | Compiled rlib inside `target/release/`. Capability-driven Govee cloud light/device telemetry and controls. |
| connector-music-assistant | `pnpm build:connector-music-assistant` | Compiled rlib inside `target/release/`. Reconnecting Music Assistant WebSocket transport; connector/media capability mapping follows separately. |
...
`loom-connector-govee`, `loom-connector-music-assistant`, `loom-connector-pihole`, `loom-connector-tasmota`, `loom-connector-truenas`,
...
pulled on either architecture. Both release workflows build AMD64 and ARM64
in parallel on matching GitHub-hosted runners, cache each architecture
separately in GHCR, then join the two immutable digests into the version and
`latest` manifest tags. This avoids QEMU entirely, most importantly keeping the
ARM64 Rust compiler off the much slower emulation path.
